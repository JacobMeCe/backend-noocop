...
interface OrdenesCompraMesReportValues {
    fechaInicio: Date;
    fechaFin: Date;
    totalOrdenes: number;
...
export const getOrdenesCompraMesReport = (
    values: OrdenesCompraMesReportValues,
): TDocumentDefinitions => {
    const { fechaInicio, fechaFin, totalOrdenes, ordenes } = values;
    ...
    const docDefinition: TDocumentDefinitions = {
        ...
        content: [
            {
                text: 'ÓRDENES DE COMPRA POR PERÍODO',
                style: 'header',
            },
            {
            ...
            // Tabla de órdenes
            {
                table: {
                    headerRows: 1,
                    widths: [50, 45, '*', 60, 55, 45, 50, 45, 50],
                    body: tableBody,
                },
                layout: {
                    fillColor: function (rowIndex, node, columnIndex) {
                        // No aplicar fillColor a la columna de estado (índice 5)
